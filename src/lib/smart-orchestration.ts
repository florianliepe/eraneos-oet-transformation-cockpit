...
export type OrchestrationPlan = {
  policyVersion: "smart-routing-1.2.0"; evidenceSufficient: boolean; selectedWorkflows: AgentWorkflowId[]; decisions: RoutingDecision[];
...
const UNIT = { estimatedTokens: 1800, estimatedCostEur: 0.018, estimatedLatencyMs: 18000 };
const CONSOLIDATION_LATENCY_MS = 8000;
const rules: Array<{ workflowId: AgentWorkflowId; terms: RegExp; reason: string }> = [
  ...
  { workflowId: "governance.review", terms: /audit finding|compliance breach|policy exception|segregation of duties|conflict of interest/i, reason: "A material governance exception requires specialist analysis before accountable human review." },
];
...
export function planAgentRoute(input: {
  ...
  return {
    policyVersion: "smart-routing-1.2.0", evidenceSufficient: true, selectedWorkflows: within, decisions,
