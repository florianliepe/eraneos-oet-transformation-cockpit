...
export type OrchestrationPlan = {
  policyVersion: "smart-routing-1.0"; evidenceSufficient: boolean; selectedWorkflows: AgentWorkflowId[]; decisions: RoutingDecision[];
...
const UNIT = { estimatedTokens: 1800, estimatedCostEur: 0.018, estimatedLatencyMs: 8000 };
...
export function planAgentRoute(input: {
  ...
  const within = ordered.filter((_, index) => index < budget.maxSpecialists && (index + 1) * UNIT.estimatedTokens <= budget.maxTokens && (index + 1) * UNIT.estimatedCostEur <= budget.maxCostEur && (index + 1) * UNIT.estimatedLatencyMs <= budget.maxLatencyMs);
  ...
  return {
    policyVersion: "smart-routing-1.0", evidenceSufficient: true, selectedWorkflows: within, decisions,
    budget: { ...budget, estimatedTokens: within.length * UNIT.estimatedTokens, estimatedCostEur: within.length * UNIT.estimatedCostEur, estimatedLatencyMs: within.length * UNIT.estimatedLatencyMs, limited },
