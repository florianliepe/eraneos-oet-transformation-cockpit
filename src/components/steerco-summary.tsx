...
export function SteercoWorkbench({ pmo, workspaceSecret }: { pmo: PmoDocument; workspaceSecret: string }) {
  ...
  return <div className="steerco-workbench">
    ...
    {snapshot && <><SteercoReport snapshot={snapshot} editable={snapshot.status === "draft"} onEditClaim={editClaim}/>
      ...
      {snapshot.status === "published" && <section className="panel steerco-share"><div><span className="section-kicker">READ-ONLY LINK ACTIVE</span><h3>Approved SteerCo view is ready</h3><p>Snapshot {snapshot.id} · checksum {snapshot.publication.checksum || "verified by publisher"}</p></div><button className="button primary" onClick={() => void copyLink()}>Copy read-only link</button><button className="button secondary" onClick={() => window.print()}>Print / Save as PDF</button><button className="button secondary" onClick={() => downloadSteercoPowerPoint(snapshot)}>Download PowerPoint</button><button className="button secondary" disabled={busy} onClick={() => void rollback()}>Restore prior release</button><button className="button ghost" disabled={busy} onClick={() => void revoke()}>Revoke link</button></section>}
    </>}
  </div>;
}
