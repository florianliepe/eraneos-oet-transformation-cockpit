...
import { applySteercoApproval, applySteercoRagOverride, assertSteercoSourcesCurrent, buildSteercoEnvelope, buildSteercoEvidence, rejectSteercoDraft, resolveSteercoPeriod, type SteercoClaim, type SteercoModule, type SteercoPeriod, type SteercoRag, type SteercoSnapshot } from "@/lib/steerco-schema";
import { downloadSteercoPowerPoint } from "@/lib/steerco-export";
import { appendPublicationReceipt, assertExecutiveReportPublishable, buildExecutiveReportPackage, recordExecutiveReview, reportSourceFingerprint, type ExecutiveReportPackage } from "@/lib/executive-reporting";
...
const sectionLabels: Record<keyof SteercoSnapshot["sections"], string> = {
  ...
  benefits: "Benefits", finances: "Finances", scenarios: "Scenarios",
};
const reportModules: Array<{ value: SteercoModule; label: string }> = [
  { value: "delivery", label: "Delivery" }, { value: "risks", label: "Risks" }, { value: "decisions", label: "Decisions" },
  { value: "benefits", label: "Benefits" }, { value: "finances", label: "Finances" }, { value: "scenarios", label: "Scenarios" },
];
...
export function SteercoWorkbench({ pmo, workspaceSecret, workspaceScope }: { pmo: PmoDocument; workspaceSecret: string; workspaceScope: WorkspaceScope }) {
  ...
  const [reviewer, setReviewer] = useState("Programme Sponsor");
  const [decisionRequest, setDecisionRequest] = useState("Confirm recovery action and delivery tolerance.");
  const [snapshot, setSnapshot] = useState<SteercoSnapshot | null>(null);
  const [reportPackage, setReportPackage] = useState<ExecutiveReportPackage | null>(null);
  ...
  const [selectedModules, setSelectedModules] = useState<SteercoModule[]>(reportModules.map((item) => item.value));
  ...
  function changePreset(value: SteercoPeriod["preset"]) {
    setPreset(value); const resolved = resolveSteercoPeriod(value, pmo); setFrom(resolved.from); setTo(resolved.to); setSnapshot(null); setReportPackage(null);
  }
  async function generate() {
    setBusy(true); setError(""); setMessage("");
    try {
      const envelope = buildSteercoEnvelope(pmo, modules, period);
      const evidence = buildSteercoEvidence(envelope, actor.trim(), selectedModules);
  ...
  async function approve() {
    ...
    try {
      ...
      const locallyApproved = applySteercoApproval(snapshot, reviewer, approvalReason);
      const assigned = buildExecutiveReportPackage({ snapshot: locallyApproved, scope: { organisationId: workspaceScope.organisationId, targetKind: "project", targetIds: [workspaceScope.projectId] }, reviewer, decisionRequests: [{ id: `decision-${locallyApproved.id}`, title: decisionRequest.trim(), owner: reviewer.trim(), dueDate: locallyApproved.period.to, status: "requested", sourceIds: locallyApproved.sections.decisions.flatMap((item) => item.sourceIds) }] });
      const reviewed = recordExecutiveReview(assigned, reviewer, "approved", approvalReason);
      assertExecutiveReportPublishable(reviewed, locallyApproved);
      const response = await approveSteercoSnapshot(workspaceSecret, locallyApproved, reviewer, approvalReason, snapshot.revision);
      const approvedSnapshot = response.snapshot || locallyApproved;
      setSnapshot(approvedSnapshot); setReportPackage(appendPublicationReceipt({ ...reviewed, snapshot: approvedSnapshot, sourceFingerprint: reportSourceFingerprint(approvedSnapshot) }, "approved", reviewer, approvalReason)); setMessage("SteerCo snapshot approved and locked for publication.");
  ...
  async function publish() {
    ...
    try { assertSteercoSourcesCurrent(snapshot, pmo, modules); if (!reportPackage) throw new Error("The assigned review receipt is unavailable; approve a new report."); assertExecutiveReportPublishable(reportPackage, snapshot); const response = await publishSteercoSnapshot(workspaceSecret, snapshot, expiresAt ? new Date(`${expiresAt}T23:59:59Z`).toISOString() : undefined); if (!response.snapshot) throw new Error("Publication returned no immutable snapshot receipt."); setSnapshot(response.snapshot); setReportPackage(appendPublicationReceipt({ ...reportPackage, snapshot: response.snapshot, sourceFingerprint: reportSourceFingerprint(response.snapshot) }, "published", reviewer)); setMessage("Read-only SteerCo snapshot published."); }
  ...
  async function revoke() {
    ...
    setBusy(true); setError(""); try { const response = await revokeSteercoShare(workspaceSecret, shareId, actor, reason); if (response.snapshot) { setSnapshot(response.snapshot); if (reportPackage) setReportPackage(appendPublicationReceipt({ ...reportPackage, snapshot: response.snapshot }, "revoked", actor, reason)); } setMessage("Read-only link revoked."); } catch (cause) { setError(cause instanceof Error ? cause.message : "Revocation failed."); } finally { setBusy(false); }
  }
  async function rollback() {
    ...
    try {
      ...
      setSnapshot(response.snapshot); if (reportPackage) setReportPackage(appendPublicationReceipt({ ...reportPackage, snapshot: response.snapshot, sourceFingerprint: reportSourceFingerprint(response.snapshot) }, "restored", actor, reason)); setMessage(`Revision ${targetRevision} restored as a new read-only publication.`);
  ...
  return <div className="steerco-workbench">
    ...
    {snapshot && <><SteercoReport snapshot={snapshot} editable={snapshot.status === "draft"} onEditClaim={editClaim}/>
      ...
      {snapshot.status === "published" && <section className="panel steerco-share"><div><span className="section-kicker">READ-ONLY LINK ACTIVE</span><h3>Approved SteerCo view is ready</h3><p>Snapshot {snapshot.id} · checksum {snapshot.publication.checksum || "verified by publisher"}</p></div><button className="button primary" onClick={() => void copyLink()}>Copy read-only link</button><button className="button secondary" onClick={() => window.print()}>Print / Save as PDF</button><button className="button secondary" onClick={() => downloadSteercoPowerPoint(snapshot, reportPackage || undefined)}>Download PowerPoint</button><button className="button secondary" disabled={busy} onClick={() => void rollback()}>Restore prior release</button><button className="button ghost" disabled={busy} onClick={() => void revoke()}>Revoke link</button></section>}
      {reportPackage && <section className="panel report-publication-history"><header><span className="section-kicker">IMMUTABLE REPORT LINEAGE</span><h3>Review and publication receipts</h3><p>{reportPackage.scope.organisationId} · {reportPackage.scope.targetIds.join(", ")} · {reportPackage.sourceFingerprint}</p></header>{reportPackage.publicationHistory.map((receipt) => <article key={receipt.id}><b>{receipt.action}</b><span>{receipt.actor} · {formatDate(receipt.at)}</span><code>r{receipt.snapshotRevision} · {receipt.sourceFingerprint}</code><small>{receipt.reason || "No additional rationale required."}</small></article>)}</section>}
    </>}
  </div>;
}

export function SteercoReadOnly({ snapshot }: { snapshot: SteercoSnapshot }) {
  return <main className="steerco-readonly"><header className="steerco-readonly-bar"><div><BrandMark compact/><b>Steering Committee summary</b></div><div><span>View only</span><button className="button secondary" onClick={() => window.print()}>Print / Save as PDF</button><button className="button secondary" onClick={() => downloadSteercoPowerPoint(snapshot)}>Download PowerPoint</button></div></header><SteercoReport snapshot={snapshot} editable={false}/><footer>Immutable approved snapshot · No editing authority · Generated data is limited to SteerCo visibility.</footer></main>;
}
...
function SteercoReport({ snapshot, editable, onEditClaim }: { snapshot: SteercoSnapshot; editable: boolean; onEditClaim?: (id: string, text: string) => void }) {
  return <article className="steerco-report" aria-label="Steering Committee project summary">
    ...
    <section className="panel steerco-trends"><header><span className="section-kicker">PERIOD COMPARISON</span><h3>Trends and material changes</h3></header><div className="steerco-trend-grid">{snapshot.reporting.trends.map((trend) => <article key={trend.id}><span>{trend.label}</span><b>{trend.current.toLocaleString("en-GB")} {trend.unit}</b><small className={`trend ${trend.direction}`}>{trend.direction}{trend.previous === undefined ? " · first governed period" : ` · previous ${trend.previous.toLocaleString("en-GB")}`}</small><p>{trend.explanation}</p></article>)}</div>{snapshot.reporting.materialChanges.map((item) => <div className={`steerco-claim ${item.kind}`} key={item.id}><p>{item.text}</p><ClaimMeta claim={item}/></div>)}</section>
    <section className="steerco-section-grid">{(Object.keys(sectionLabels) as Array<keyof SteercoSnapshot["sections"]>).filter((key) => sectionEnabled(key, snapshot.reporting.modules)).map((key) => <article className="panel" key={key}><header><span className="section-kicker">{key.replace(/([A-Z])/g, " $1").toUpperCase()}</span><h3>{sectionLabels[key]}</h3></header>{snapshot.sections[key].map((item) => <div className={`steerco-claim ${item.kind}`} key={item.id}><p>{item.text}</p><ClaimMeta claim={item}/></div>)}</article>)}</section>
  </article>;
}

function sectionEnabled(key: keyof SteercoSnapshot["sections"], modules: SteercoModule[]) {
  if (["milestones", "deliverables", "overdueActions", "dependencies", "assumptions", "changeRequests", "upcoming", "changes", "moduleHighlights", "governance", "automation"].includes(key)) return modules.includes("delivery");
  if (key === "risks" || key === "issues") return modules.includes("risks");
  if (key === "decisions") return modules.includes("decisions");
  return modules.includes(key as SteercoModule);
}
