...
export function ProposalReviewInbox({ proposalSets, busy, onSubmit }: {
  ...
  const decisionFor = (proposalId: string) => decisions[proposalId] || { rationale: "" };
  ...
  const invalid = active.proposals.filter((proposal) => {
    const decision = decisionFor(proposal.id);
    return !decision.decision || (proposal.risk === "high" && decision.rationale.trim().length < 20);
  });
  const submit = async () => onSubmit(active, reviewer, active.proposals.map((proposal) => {
    const decision = decisionFor(proposal.id);
    return { proposalId: proposal.id, decision: decision.decision!, rationale: decision.rationale };
  }));
  ...
  return <div className="proposal-review-layout">
    ...
    <section className="proposal-review-main">
      ...
      {active.proposals.map((proposal) => { const decision = decisionFor(proposal.id); return <article className="panel proposal-review-card" key={proposal.id}>
        ...
        <footer><span>{proposal.evidenceIds.length} evidence reference{proposal.evidenceIds.length === 1 ? "" : "s"}</span><label><span>Decision rationale {proposal.risk === "high" && "(required, minimum 20 characters)"}</span><textarea aria-invalid={proposal.risk === "high" && decision.rationale.trim().length < 20} value={decision.rationale} onChange={(event) => setDecision(proposal.id, { rationale: event.target.value })} placeholder="Record evidence reviewed and the reason for this decision."/>{proposal.risk === "high" && decision.rationale.trim().length < 20 && <small role="alert">Add at least 20 characters of accountable rationale.</small>}</label></footer>
      </article>; })}
      <div className="proposal-review-actions"><p>{invalid.length ? `Complete a decision and required rationale for ${invalid.length} proposal${invalid.length === 1 ? "" : "s"}.` : "Rejected proposals remain immutable and never change canonical state. Accepted proposals create one governed revision after the publisher gate."}</p><button className="button primary" disabled={busy || active.status !== "pending_review" || !reviewer.trim() || invalid.length > 0} onClick={() => void submit()}>{busy ? "Publishing governed decision..." : "Record review and publish accepted"}</button></div>
    </section>
  </div>;
}
