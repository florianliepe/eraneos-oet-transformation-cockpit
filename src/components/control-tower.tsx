...
function AccessDialog({ loading, error, onUnlock }: { loading: boolean; error: string; onUnlock: (secret: string) => void }) {
  ...
  return <div className="modal-backdrop"><form className="modal publish-modal" onSubmit={(event) => { event.preventDefault(); onUnlock(secret); }}><header><div><span className="section-kicker">WORKFLOW ACCESS</span><h2>Connect the project data boundary</h2></div></header><p>Your local account session is active. This separate temporary credential authorises the existing governed workflow endpoint until the App Service API can enforce workspace access server-side.</p>{error && <div className="error-banner" role="alert"><span>{error}</span></div>}<label><span>Temporary workspace credential</span><input type="password" autoFocus required value={secret} onChange={(event) => setSecret(event.target.value)} autoComplete="current-password"/><small>Never reused as your account password. Kept in memory only and cleared when this page closes or refreshes.</small></label><footer><button className="button primary" disabled={loading}>{loading ? "Connecting..." : "Open workspace"}</button></footer></form></div>;
}
