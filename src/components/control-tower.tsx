...
export default function ControlTower({ initialData }: { initialData: PmoDocument }) {
  ...
  if (shareRequested && !shareError) return <div className="app-loading"><BrandMark/><div className="loading-line"/><p>Loading approved SteerCo snapshotâ€¦</p></div>;
  ...
  if (loading) return <div className="app-loading"><BrandMark/><div className="loading-line"/><p>Connecting to the project control towerâ€¦</p></div>;
...
function Overview({ data, exposure, openActions, completedDeliverables, setView, onEdit, onDelete }: { data: PmoDocument; exposure: number; openActions: number; completedDeliverables: number; setView: (view: View) => void; onEdit: (target: EditorTarget) => void; onDelete: (entity: "workstream", id: string, label: string) => void }) {
  ...
  return <>
    <section className="project-hero"><div><div className="hero-top"><RagDot rag={data.project.overallRag}/><span>Revision {data.revision}</span></div><h2>{data.project.name}</h2><p>{data.project.subtitle} Â· {data.project.phase}</p><button className="hero-edit" onClick={() => onEdit({ entity: "project", id: data.project.id })}><Icons.edit/>Edit project profile</button></div><div className="hero-progress"><div><b>{data.project.progress}%</b><span>overall progress</span></div><ProgressBar value={data.project.progress} tone={data.project.overallRag}/><small>{formatDate(data.project.startDate)} â€” {formatDate(data.project.endDate)}</small></div></section>
    <section className="metric-grid">
      ...
      <article className="metric-card"><span>ACTIVE EXPOSURE</span><div><strong>{exposure}</strong><em className="trend warn">{highRisks.length} high</em></div><div className="mini-bars">{data.risks.map((risk) => <i key={risk.id} style={{ height: `${Math.max(18, riskScore(risk) * 3.3)}%` }} className={riskScore(risk) >= 16 ? "bar-red" : riskScore(risk) >= 10 ? "bar-amber" : "bar-green"}/>)}</div><small>Weighted probability Ã— impact</small></article>
