...
const release = {
  ...
  releaseId: "2026-08-13-zm-prod-14-smart-routing",
  ...
  recoveryEvidence: { nonProductionWorkflowId: "0ZJDa3r5hGoB9SLq", result: "success", assertion: "The live smart-routing canary must preserve canonicalWriteAllowed=false; routine PMO intake omits the redundant governance model while material policy exceptions still select it, and rejected or duplicate publication remains shouldWrite=false" }
};
mkdirSync(resolve("docs/n8n/releases"), { recursive: true });
writeFileSync(resolve("docs/n8n/releases/2026-08-13-zm-prod-14-smart-routing.json"), `${JSON.stringify(release, null, 2)}\n`);
