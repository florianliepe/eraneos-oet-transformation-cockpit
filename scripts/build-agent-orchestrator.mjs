...
const routed=selected.filter((id,index)=>index<maxSpecialists&&(index+1)*1800<=maxTokens&&(index+1)*.018<=maxCost&&(index+1)*18000+8000<=maxLatency); const limited=routed.length<selected.length;
return routed.map((workflowId,index)=>({json:{contractVersion:'agent-run-1.0',executionId,correlationId,requestedAt,meta:source.meta,evidence,workflowId,liveWorkflowId:bindings[workflowId],routingPlan:{policyVersion:'smart-routing-1.2.0',workflowId,sequence:index+1,reason:manual?'Accountable manual override':workflowId==='evidence.verify'?'Evidence verification precedes dependent analysis':(rules.find(rule=>rule[0]===workflowId)?.[2]||'Specialist analysis selected by scoped evidence'),budget:{maxTokens,maxCostEur:maxCost,maxLatencyMs:maxLatency,estimatedTokens:routed.length*1800,estimatedCostEur:routed.length*.018,estimatedLatencyMs:routed.length*18000+8000,limited},manualOverride:manual?{actor:String(source.meta.manual_override_actor),reason:String(source.meta.manual_override_reason)}:undefined}}}));`;
...
const plan=aggregate.routingPlan||[];const budget=plan[0]?.budget;const agentRun={contractVersion:'agent-run-1.0',executionId:aggregate.executionId,correlationId:aggregate.correlationId,requestedAt:aggregate.requestedAt,completedAt,status,orchestrator:{workflowId:'pmo.orchestrate',workflowVersion:'1.3.2'},routing:{mode:String(aggregate.meta?.manual_override_reason?'manual':'smart_auto'),selectedWorkflows:aggregate.selectedWorkflows,policyVersion:'smart-routing-1.2.0',explanation:plan.map(item=>({workflowId:item.workflowId||aggregate.selectedWorkflows[item.sequence-1],reason:item.reason,sequence:item.sequence})),budget,manualOverride:plan[0]?.manualOverride},steps:aggregate.steps,evidence,proposals:aggregate.proposals,warnings:aggregate.warnings,persistence:{mode:'proposal_only'}};
...
await import(`./harden-governed-publisher.mjs?build=${Date.now()}`);
