...
test("steers two projects from the organisation dashboard", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Project Alpha" })).toBeVisible(); await expect(page.getByRole("heading", { name: "Project Beta" })).toBeVisible();
  const beta = page.getByRole("article").filter({ hasText: "Project Beta" }); await beta.getByRole("button", { name: "Open cockpit" }).click(); await expect(page.getByText("Project Beta", { exact: true }).first()).toBeVisible();
  await page.getByRole("button", { name: "Organisation workspace" }).click(); const alpha = page.getByRole("article").filter({ hasText: "Project Alpha" }); await alpha.getByRole("button", { name: "Archive" }).click(); await expect(alpha.getByText("archived")).toBeVisible(); await alpha.getByRole("button", { name: "Restore" }).click(); await expect(alpha.getByText("active")).toBeVisible();
});
